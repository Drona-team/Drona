  ...
  implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider
{
  ...
  private void layoutForPredictiveAnimations(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState, int paramInt1, int paramInt2)
  {
    if ((paramState.willRunPredictiveAnimations()) && (getChildCount() != 0) && (!paramState.isPreLayout()))
    {
      ...
      if (j > 0)
      {
        ...
        mLayoutState.mExtraFillSpace = j;
      ...
      if (k > 0)
      {
        ...
        mLayoutState.mExtraFillSpace = k;
        mLayoutState.mAvailable = 0;
        mLayoutState.assignPositionFromScrapList();
        fill(paramRecycler, mLayoutState, paramState, false);
      }
  ...
  private void recycleViewsFromEnd(RecyclerView.Recycler paramRecycler, int paramInt1, int paramInt2)
  {
    ...
    if (mShouldReverseLayout)
    {
      ...
      for (;;)
      {
        ...
        if ((mOrientationHelper.getDecoratedStart(localView) < i) || (mOrientationHelper.getTransformedStartWithDecoration(localView) < i)) {
          break;
        }
        paramInt1 += 1;
      }
  ...
  private void updateLayoutState(int paramInt1, int paramInt2, boolean paramBoolean, RecyclerView.State paramState)
  {
    ...
    calculateExtraLayoutSpace(paramState, mReusableIntPair);
    int m = Math.max(0, mReusableIntPair[0]);
    int j = m;
    int k = Math.max(0, mReusableIntPair[1]);
    if (paramInt1 == 1) {
      i = 1;
    }
    ...
    {
      paramState = mLayoutState;
      mExtraFillSpace += mOrientationHelper.getEndPadding();
    ...
    else
    {
      ...
      localObject = mLayoutState;
      mExtraFillSpace += mOrientationHelper.getStartAfterPadding();
      localObject = mLayoutState;
      if (mShouldReverseLayout) {
        paramInt1 = 1;
      }
  ...
  int fill(RecyclerView.Recycler paramRecycler, LayoutState paramLayoutState, RecyclerView.State paramState, boolean paramBoolean)
  {
    ...
    int i = mAvailable + mExtraFillSpace;
  ...
  public void onLayoutChildren(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState)
  {
    ...
    mReusableIntPair[1] = 0;
    calculateExtraLayoutSpace(paramState, mReusableIntPair);
    int m = Math.max(0, mReusableIntPair[0]) + mOrientationHelper.getStartAfterPadding();
    int n = Math.max(0, mReusableIntPair[1]) + mOrientationHelper.getEndPadding();
    int i = m;
    int j = n;
    if (paramState.isPreLayout())
    {
    ...
    if (mAnchorInfo.mLayoutFromEnd)
    {
      ...
      mLayoutState.mExtraFillSpace = i;
      fill(paramRecycler, mLayoutState, paramState, false);
