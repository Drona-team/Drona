  ...
  implements NestedScrollingParent2, NestedScrollingParent3
{
      ...
      else
      {
        ...
        else
        {
          ...
          if (localObject != null)
          {
            ...
            ((Behavior)localObject).onNestedPreScroll(this, localView, paramView, paramInt1, paramInt2, mBehaviorConsumed, paramInt3);
            if (paramInt1 > 0) {
              i = Math.max(i, mBehaviorConsumed[0]);
            } else {
              i = Math.min(i, mBehaviorConsumed[0]);
            }
            if (paramInt2 > 0) {
              j = Math.max(j, mBehaviorConsumed[1]);
            } else {
              j = Math.min(j, mBehaviorConsumed[1]);
            }
            i1 = 1;
            m = j;
            n = i;
    ...
    while (k < n)
    {
      ...
      if (localView.getVisibility() != 8)
      {
        ...
        do
        {
          ...
          } while (!((LayoutParams)localObject).isNestedScrollAccepted(paramInt5));
          localObject = ((LayoutParams)localObject).getBehavior();
        } while (localObject == null);
        mBehaviorConsumed[0] = 0;
        mBehaviorConsumed[1] = 0;
        ((Behavior)localObject).onNestedScroll(this, localView, paramView, paramInt1, paramInt2, paramInt3, paramInt4, paramInt5, mBehaviorConsumed);
        if (paramInt3 > 0) {
          i = Math.max(i, mBehaviorConsumed[0]);
        } else {
          i = Math.min(i, mBehaviorConsumed[0]);
  ...
  public boolean onTouchEvent(MotionEvent paramMotionEvent)
  {
    ...
    do
    {
      ...
      break;
      bool1 = false;
      localObject = ((LayoutParams)mBehaviorTouchView.getLayoutParams()).getBehavior();
      bool2 = bool1;
    } while (localObject == null);
    boolean bool3 = ((Behavior)localObject).onTouchEvent(this, mBehaviorTouchView, paramMotionEvent);
    boolean bool2 = bool1;
    boolean bool1 = bool3;
  ...
  {
    throw new Error("Unresolved compilation error: Method <androidx.coordinatorlayout.widget.CoordinatorLayout: void saveAttributeDataForStyleable(android.content.Context,int[],android.util.AttributeSet,android.content.res.TypedArray,int,int)> does not exist!");
  }
