...
final class BillingResults
{
  ...
  static final BillingResult seq;
  
  static
  {
    INTERNAL_ERROR = BillingResult.newBuilder().setResponseCode(6).setDebugMessage("An internal error occurred.").build();
    ITEM_UNAVAILABLE = BillingResult.newBuilder().setResponseCode(4).setDebugMessage("Item is unavailable for purchase.").build();
    NULL_SKU = BillingResult.newBuilder().setResponseCode(5).setDebugMessage("SKU can't be null.").build();
    NULL_SKU_TYPE = BillingResult.newBuilder().setResponseCode(5).setDebugMessage("SKU type can't be null.").build();
    seq = BillingResult.newBuilder().setResponseCode(0).build();
  }
