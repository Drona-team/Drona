  ...
  implements ImageFormat.FormatChecker
{
  ...
  final int MAX_HEADER_LENGTH = Ints.min(new int[] { 21, 20, JPEG_HEADER_LENGTH, PNG_HEADER_LENGTH, 6, BMP_HEADER_LENGTH, ICO_HEADER_LENGTH, HEIF_HEADER_LENGTH });
  ...
  static
  {
    ...
    HEIF_HEADER_LENGTH = ImageFormatCheckerUtils.asciiBytes(localStringBuilder.toString()).length;
  }
  ...
  {
    if (paramInt < ICO_HEADER.length) {
      return false;
    }
    return ImageFormatCheckerUtils.startsWithPattern(paramArrayOfByte, ICO_HEADER);
  }
