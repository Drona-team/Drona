  ...
  implements CacheEvictor, Comparator<CacheSpan>
{
  ...
  {
    while (currentSize + paramLong > maxBytes)
    {
      if (leastRecentlyUsed.isEmpty()) {
        return;
      }
      Object localObject = leastRecentlyUsed;
      try
      {
        localObject = ((TreeSet)localObject).first();
        localObject = (CacheSpan)localObject;
        paramCache.removeSpan((CacheSpan)localObject);
      }
