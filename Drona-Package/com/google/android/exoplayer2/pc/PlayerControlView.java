  ...
  private void updateProgress()
  {
    if (isVisible())
    {
      ...
      if (player != null)
      {
        ...
        if (!localTimeline.isEmpty())
        {
          int k = player.getCurrentWindowIndex();
          ...
          i = k;
          if (multiWindowTimeBar) {
            i = localTimeline.getWindowCount() - 1;
          }
          int i2 = i;
          l1 = 0L;
          i = 0;
          for (l3 = 0L;; l3 = l2)
          {
            ...
            l2 = l3;
            if (j == k) {
              l2 = IpAddress.usToMs(l1);
            }
            localTimeline.getWindow(j, window);
            ...
            m = window.firstPeriodIndex;
            while (m <= window.lastPeriodIndex)
            {
              localTimeline.getPeriod(m, period);
              int i3 = period.getAdGroupCount();
              int n = 0;
              while (n < i3)
              {
                l4 = period.getAdGroupTimeUs(n);
                l3 = l4;
                  ...
                  if (l3 <= window.durationUs)
                  {
                    if (i == adGroupTimesMs.length)
                    {
                      if (adGroupTimesMs.length == 0) {
                        ...
                        i1 = adGroupTimesMs.length * 2;
                      }
                      adGroupTimesMs = Arrays.copyOf(adGroupTimesMs, i1);
                      playedAdGroups = Arrays.copyOf(playedAdGroups, i1);
                    }
                    adGroupTimesMs[i] = IpAddress.usToMs(l3 + l1);
                    playedAdGroups[i] = period.hasPlayedAdGroup(n);
                    i1 = i + 1;
                  }
                }
                label418:
        ...
        if (timeBar != null)
        {
          ...
          System.arraycopy(extraAdGroupTimesMs, 0, adGroupTimesMs, m, i);
          System.arraycopy(extraPlayedAdGroups, 0, playedAdGroups, m, i);
